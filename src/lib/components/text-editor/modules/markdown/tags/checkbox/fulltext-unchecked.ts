...
import type { Options, Tag, TagImpl, TagPattern } from '../../types.js';
import meta from '../meta.js';

class Checkbox extends AbstractTag implements TagImpl {
	quillJS: Quill;
	name: string;
	pattern: TagPattern;
		...
		return {
			...
			action: (text, selection, pattern) =>
				new Promise((resolve) => {
					...
					setTimeout(() => {
						let replaceText =
							text.split('[ ] ').length > 1 ? text.split('[ ] ').splice(1, 1).join('') : text;
						replaceText =
							replaceText.split('[] ').length > 1
								? replaceText.split('[] ').splice(1, 1).join('')
								: replaceText;
						this.quillJS.insertText(index, replaceText, { list: 'unchecked' }, 'user');
